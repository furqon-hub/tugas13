...
class BukuController extends Controller
{
    ...
    public function bulkDelete(Request $request)
    {
        ...
        if (empty($ids)) {
            return redirect()->back()->with('error', 'Silakan pilih buku yang ingin dihapus terlebih dahulu!');
        }

        Buku::whereIn('id', $ids)->delete();

        return redirect()->route('buku.index')
            ->with('success', count($ids) . ' buku berhasil dihapus secara massal!');
    }

    /**
     * Fitur Export Buku ke CSV (Tugas 3)
     */
